...
class Generator(nn.Module):
  def __init__(self, input_dim = 10, hidden_dim = 1, image_channels = 64):
...
def _generator_block(self, input_channels, output_channels, kernel_size = 3, stride = 2, final_layer = False):
  ...
  def creaate_noise_vector(n_samples, input_dim, device = 'cpu'):
  ...
  class Discriminator(nn.Module):
    def __init__(self, input_dim = 64, hidden_dim = 1):
      super(Discriminator, self).__init__()
      self.disc = nn.Sequential(
        self.discriminator_block(input_dim, hidden_dim * 2),
        self.discriminator_block(hidden_dim * 2, hidden_dim * 4),
        self.discriminator_block(hidden_dim * 4, 1, final_layer = True),
      )

    def Discriminator(self, image_channel=1, hidden_dim = 64):
      super(Discriminator, self).__init__()
      self.disc = nn.Sequential(
